...
void main() async {
  ...
  final sourceFile = File('lib/l10n/app_kk.arb');
  if (!sourceFile.existsSync()) {
    stderr.writeln('Ошибка: файл lib/l10n/app_kk.arb не найден.');
    exit(1);
  }

  final Map<String, dynamic> kkData = jsonDecode(sourceFile.readAsStringSync()) as Map<String, dynamic>;

  final keysToTranslate   = <String>[];
  final textsToTranslate  = <String>[];
  final Map<String, String> descriptionsToTranslate = {};

  for (final entry in kkData.entries) {
    final key   = entry.key;
    final value = entry.value;

    if (key.startsWith('@@')) {
      // Сохраняем метаданные @@ как есть, потом пропишем @@locale = 'ru'
  ...
  for (final entry in kkData.entries) {
    ...
    if (key.startsWith('@@')) {
      ruData[key] = (key == '@@locale') ? 'ru' : value;
    } else if (key.startsWith('@')) {
      if (translatedDescMap.containsKey(key)) {
        ruData[key] = {'description': translatedDescMap[key]};
      }
    } else {
      final idx = keysToTranslate.indexOf(key);
      ruData[key] = (idx < translatedTexts.length && translatedTexts[idx].isNotEmpty)
          ? translatedTexts[idx]
          : value;
      
      final metaKey = '@$key';
      if (kkData.containsKey(metaKey)) {
        if (translatedDescMap.containsKey(metaKey)) {
          ruData[metaKey] = {'description': translatedDescMap[metaKey]};
        }
      }
    }
  }

  final destinationFile = File('lib/l10n/app_ru.arb');
  destinationFile.writeAsStringSync(
    const JsonEncoder.withIndent('  ').convert(ruData),
  );

  stdout.writeln('\n✓ Готово! Файл lib/l10n/app_ru.arb сохранён. Переведено ключей: ${keysToTranslate.length}.');
}
