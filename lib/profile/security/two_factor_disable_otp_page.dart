...
class TwoFactorDisableOtpPage extends StatefulWidget {
  const TwoFactorDisableOtpPage({super.key, this.onDisabled});

  /// Колбэк успешного выключения. Вызывается после pop этого экрана.
  final VoidCallback? onDisabled;

  @override
  State<TwoFactorDisableOtpPage> createState() =>
      _TwoFactorDisableOtpPageState();
}

class _TwoFactorDisableOtpPageState extends State<TwoFactorDisableOtpPage> {
  ...
  Future<void> _requestInitialOtp() async {
    ...
    // Если ранее запрос уже был и cooldown не истёк - не дёргаем API,
    // просто запускаем таймеры от сохранённого момента. Сервер всё равно
    // вернул бы тот же OTP, а пользователь уже получил письмо.
    ...
    try {
      ...
      if (!mounted) return;
      await OtpCooldownStore.markRequested(userId.toString(), 'disable');
      _startTtlCountdown();
  ...
  Future<void> _submit() async {
    ...
    // Снимок userId/email ДО вызова verifyDisable - после успешного disable
    // серверная запись trusted-устройства revoked=true, локальные device-токены
    // тоже больше не нужны и должны быть удалены.
    final snapshotUserId = AuthStorage.userId ?? 0;
    final snapshotEmail = AuthStorage.email ?? '';

    try {
      await TwoFactorApi.verifyDisable(code);
      if (snapshotUserId > 0 || snapshotEmail.isNotEmpty) {
        await AuthStorage.clearDeviceToken(
          snapshotUserId,
          email: snapshotEmail,
        );
      }
      // Cooldown больше не нужен - 2FA отключена.
      await OtpCooldownStore.clear(snapshotUserId.toString(), 'disable');
      if (!mounted) return;
      _ttlTimer?.cancel();
      _resendTimer?.cancel();
      // Сначала pop, потом колбэк - чтобы родитель успел перечитать статус.
  ...
  Widget _buildPinCodeField() {
    ...
    // LayoutBuilder подбирает ширину ячейки под доступное пространство -
    // 6 ячеек по 65 не помещаются на узких экранах (<400dp).
